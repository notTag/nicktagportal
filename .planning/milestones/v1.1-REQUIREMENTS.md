# Requirements Archive: v1.1 CLI Remote & Site Polish

**Shipped:** 2026-04-20
**Phases:** 5-10 (Theme System, Skills Diamond Wall, CLI Terminal Core, CLI Federation, Deployment & Infrastructure, Collapsible Sidebar)

## Outcome Summary

Phase-level verification artifacts (see `v1.1-phases/*-VERIFICATION.md` once archived) confirm all in-scope requirements shipped, with the following explicit deferrals:

| Requirement | Status                 | Reason                                                             |
| ----------- | ---------------------- | ------------------------------------------------------------------ |
| SKL-03      | Deferred → Phase 999.9 | Staggered entrance animation deferred per user approval 2026-04-02 |
| SKL-08      | Deferred → Phase 999.9 | IntersectionObserver scroll trigger deferred alongside SKL-03      |
| CLI-19      | Dropped                | `resume` command dropped from scope per user decision 2026-04-20   |
| CLI-20      | Dropped                | `ssh` easter egg dropped from scope per user decision 2026-04-20   |

Checkbox state below reflects execution-time bookkeeping; phase VERIFICATION.md files are the authoritative record of what shipped in code.
