...
import { createFilesystem, resolvePath, getNode, HOME_DIR } from './vfs/filesystem'
...
export class Shell {
  ...
  private writePrompt(): void {
    const displayPath = this.cwd === HOME_DIR
      ? '~'
  ...
  private handleData(data: string): void {
    ...
    if (data === '\x1b[A') {
      ...
      if (this.historyIndex < this.history.length - 1) {
        this.historyIndex++
        this.replaceLineWith(this.history[this.history.length - 1 - this.historyIndex])
      }
      return
    }
  ...
  private completeFilePath(partial: string): void {
    ...
    if (matches.length === 1) {
      const completion = matches[0].name.slice(namePrefix.length) + matches[0].suffix
      this.lineBuffer += completion
      this.cursorPos += completion.length
      this.terminal.write(completion)
    } else {
  ...
  private executeCommand(input: string): void {
    ...
    if (firstWord in this.aliases) {
      const expanded = this.aliases[firstWord] + (parts.length > 1 ? ' ' + parts.slice(1).join(' ') : '')
      input = expanded
    }
    ...
    if (clithemeMatch) {
      ...
      if (themeId in themes) {
        ...
        this.terminal.writeln(ansi.red('Unknown theme: ' + rawValue + '. Available: ' + available))
      }
      return
    }
